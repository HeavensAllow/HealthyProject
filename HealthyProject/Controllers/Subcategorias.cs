...
{
    public class Subcategorias : Controller
    {
        ...
        public ActionResult CreateSubcategoria([Bind(Include = "Nome, Texto")] Subcategoria newSubcategoria, int? categoria)
        {
            var cat = db.Categorias.Find(categoria);
            if (cat == null)
            {
                return HttpNotFound();
            }

            newSubcategoria.CategoriaID = (int)categoria;
            db.Subcategorias.Add(newSubcategoria);
            db.SaveChanges();
            return RedirectToAction("Index", new { id = newSubcategoria.SubcategoriaID });
